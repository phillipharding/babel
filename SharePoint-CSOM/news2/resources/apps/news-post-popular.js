...
function getFilter() {
    /*
    $select=Title,PostCategory/Id&$expand=PostCategory/Id&$filter=PostCategory/Id eq 4
    */
    var
        now = new Date(),
        sdt = null, 
    ...
    if (cid) {
        filter += String.format(" and PostCategory/Id eq {0}", cid);
...
news.popular.getPosts = function() {
    var
        p = new $.Deferred(),
        r = {
            url: _spPageContextInfo.webServerRelativeUrl + "/_api/web/lists/getbytitle('Posts')/items?$select=Id,Title,PublishedDate,LikesCount,NumCommentsId,PostCategory/Id&$expand=PostCategory/Id&$orderby=PublishedDate desc"
                    + getFilter(),
            type: 'GET',
            headers: {
